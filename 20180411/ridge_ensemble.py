...
target = 'ru0'
train_start, train_end, test_start, test_end = '20180328', ' 20180329', '20180409', '20180409'
period = '1s'
if period == '0s':
    lag = '500ms'
else:
...
asset = trade.simu_trade1(train_price, train_predict, buy_threshold=buy_thres_ask_bid,sell_threshold=sell_thres_ask_bid
                                  , hold_per_trade=50)
plt.figure(figsize= (10,6))
plt.title('asset track of train data, trade at ask price and bid price')
plt.plot(asset)
plt.savefig(to_save_dir+'train_ask_bid_best.jpg')
# plt.show()
plt.close()
...
def easy_ensemble(data, clf, target=target,n_model='default', zero_no_zero_times = 2, cut_outlier=True):
    ...
    if cut_outlier:  # 去掉非零值中的outliers
        mean = np.mean(not_zero_records)
        std = np.std(not_zero_records)
        flag=[]
