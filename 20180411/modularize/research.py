...
class Research(object):
    ...
    def load_return_from_local(self, date, tickerlst, period, lag, resample_period='0s',sample_how='first'): # 读取对应日期、period等之下的return，并给出heatmap图、corr_mat保存到本地
        ...
        if lag == '0s':  # 是一个完整的return file,包含了各个ticker，所以是先全部读进来然后再取出tickerlst部分
            ...
            corr = tmp.corr()
            self.save_corrmat_heatmap(self.out_dir+ keywd+'/single_day_return/'+self.target+'/'+date,corr,period,lag,resample_period,date)
            return tmp
        else:  # 由于lag,取self.target文件夹
            file_dir = self.dir+'/lagged_return/'+ date+'/'+keywd+'/'+self.target+'/'+'/period_'+period+'_lag_'+lag+'.dat.gz' # 由于在计算corr的时候只取了未resample的，因此lagged return中全部未resample,所以在这里读取数据后补充resample
            tmp = pd.read_csv(file_dir, header=0, index_col=0, compression='gzip')
            ...
            corr = sampled.corr()
            self.save_corrmat_heatmap(self.out_dir + keywd + '/single_day_return/' + self.target + '/' + date, corr,
                    ...
                    for resample_period in resample_list:
                        ...
                        if len(daylst) > 1: # 单日数据的在load return from local中已经保存
                            ...
                            self.save_corrmat_heatmap(
                                self.out_dir + keywd + '/single_day_return/' + self.target + '/' + daylst[0] + '_' + daylst[
                                        -1], corr, period, lag,resample_period, daylst)
    ...
    def check_calculated(self, daylst):
        if len(daylst) == 1:
            folder_name = 'single_day_return/'+self.target+'/'+daylst[0]
        else:
            folder_name = 'multi_day_return/'+self.target+'/'+daylst[0]+'-'+daylst[-1]
        if self.type == 0:
            keywd = 'rolling'
        else:
            keywd = 'aggravated'
